...
import static org.firstinspires.ftc.teamcode.pedroPathing.tuning.FollowerConstants.sensorName;
...
public class OTOSLocalizer extends Localizer {
    ...
    private static final double sensorOffsetX = 0;
    private static final double sensorOffsetY = 0;
    ...
    // Sum of the rotation of a given direction
    // spin left = positive, spin right = negative
    ...
    public OTOSLocalizer(HardwareMap map, Pose setStartPose) {
        ...
        otos = hardwareMap.get(SparkFunOTOS.class, sensorName);
        ...
        otos.setOffset(new SparkFunOTOS.Pose2D(sensorOffsetX, sensorOffsetY,Math.PI / 2));
    ...
    public void update() {
        ...
        double adjustedAngle = MathFunctions.getSmallestAngleDifference(otosPose.h, previousHeading);
        totalHeading += adjustedAngle;

        if (previousHeading > otosPose.h) {
        rotationChange -= adjustedAngle;
        } else{
            rotationChange += adjustedAngle;
        }
    ...
    /**
     * This returns the total rotational value from turning both directions
     * turning left increases the value, turning right decreases the value.
     * i.e. turn 1800 degrees left == 31.41... then 1800 right == 0.0
     * @return the current rotational value of all directions
     */
    public double getRotationalChange() {
